...
async def get_nomenclature_placing(db: Session = Depends(get_db), token: str = None):
    if token is None:
        nomenclatures = crud.get_nomenclature_placing_from_db(db=db)
    else:
        nomenclatures = crud.get_nomenclature_placing_from_db_for_user(db=db, token=token)
