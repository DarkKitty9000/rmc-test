...
class NomenclaturePlacing(BaseModel):
    ...
    abbreviaturafedokrug: str = Field(serialization_alias='АббревиатураФедОкруг')
    ...
    dataposledneydostupnostitochki: str = Field(serialization_alias='ДатаПоследнейДоступностиТочки')
    dictorskayanachitka: str = Field(serialization_alias='ДикторскаяНачитка')
    dictorskayanachitkapoumolchaniu: bool = Field(serialization_alias='ДикторскаяНачиткаПоУмолчанию')
    dom: str = Field(serialization_alias='Дом')
    dostupnostnomenclatury: str = Field(serialization_alias='ДоступностьНоменклатуры')
    ...
    zvukovayapodlozhkapoumolchaniu: bool = Field(serialization_alias='ЗвуковаяПодложкаПоУмолчанию')
    ...
    naimenovaniefiltrovannoe: str = Field(serialization_alias='НаименованиеФильтрованное')
    operator: str = Field(serialization_alias='Оператор')
    operatoraremcy: bool = Field(serialization_alias='ОператорАремси')
    osnovnoekontaktnoelicokod: str = Field(serialization_alias='ОсновноеКонтактноеЛицоКод')
    ...
    sobstvennik: str
    statisticanomenclatury: bool = Field(serialization_alias='СтатистикаНоменклатуры')
